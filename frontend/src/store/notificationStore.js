...
export const useNotificationStore = create((set, get) => ({
  ...
  markAllRead: async () => {
    ...
    const next = normalizeNotifications(get().notifications).map(
...
function loadStoredNotifications() {
  try {
    return normalizeNotifications(
      JSON.parse(localStorage.getItem("hkick_notifications") || "null"),
    );
  } catch {
...
function normalizeNotifications(value) {
  ...
  return value.filter(Boolean).map((notification) => ({
    id:
      notification.id || `notification-${Math.random().toString(36).slice(2)}`,
