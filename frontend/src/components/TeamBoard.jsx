...
export function TeamBoard({ players = [] }) {
  ...
  const totals = teams.map((team) =>
    team.players.reduce((sum, player) => sum + player.skillLevel, 0),
  );
    ...
    <div className="space-y-3">
      <div className="rounded-lg border border-limeball/35 bg-limeball/10 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="flex items-center gap-2 text-xs font-black uppercase text-turf dark:text-limeball">
              <Bot size={15} /> AI balance engine
            </p>
            <h2 className="mt-1 text-2xl font-black">
              Squads stay fair after every join
            </h2>
          </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {teams.map((team) => (
          <section
            key={team.name}
            className="rounded-lg border border-black/10 bg-white/75 p-4 dark:border-white/10 dark:bg-white/10"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs font-black uppercase text-black/45 dark:text-white/45">
            ...
            <div className="mt-4 space-y-2">
              {team.players.map((player) => (
                <div
                  key={player.id}
                  className="flex items-center justify-between rounded-lg bg-black/[0.04] px-3 py-2.5 dark:bg-white/[0.06]"
                >
                  <div className="flex min-w-0 items-center gap-2">
                    <Shirt
                      size={16}
                      className="shrink-0 text-turf dark:text-limeball"
                    />
                  ...
                  <span className="rounded-lg bg-white px-2 py-1 text-xs font-black text-ink dark:bg-ink dark:text-limeball">
                    L{player.skillLevel}
                  </span>
                </div>
              ))}
